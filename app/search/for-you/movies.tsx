...
export default function ForYouMoviesScreen() {
  ...
  useEffect(() => {
    const generateMovieRecommendations = async () => {
      try {
        ...
        const userMovieItems = userItems.filter(item => item.media_type === 'movie');
        
        if (userMovieItems.length === 0) {
          // If no watched/watchlist movie items, fetch popular movies
          ...
          setRecommendations(popularMovies.results);
          setError(null);
          setLoading(false);
          return;
        }
        ...
        setError('Failed to load personalized movie recommendations. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Generating your movie recommendations...</Text>
      </View>
    );
  }
  ...
  if (recommendations.length === 0) {
    return (
      <View style={styles.emptyContainer}>
        <Text style={styles.emptyText}>No movie recommendations found. Try adding more movies to your watchlist.</Text>
      </View>
    );
  }
